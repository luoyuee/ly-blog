...
<template>
  <div class="text-sm leading-6 text-default">
    <div v-if="showHeader" class="mb-4 flex items-center gap-3">
      <div class="min-w-0 flex-1 truncate text-base font-semibold leading-6 text-highlighted">
        <slot name="title">{{ title }}</slot>
      </div>
      <div class="shrink-0 text-sm leading-5 text-toned">
        <slot name="extra">{{ extra }}</slot>
      </div>
    </div>

    <div v-if="vertical" class="w-full overflow-hidden rounded-md border border-muted">
      <table class="m-0 w-full table-auto border-collapse">
        <tbody>
          <template v-for="row in groupItems.length" :key="row">
            <tr ref="thVerticalBorderedRows" class="border-b border-muted last:border-b-0"></tr>
            <tr ref="tdVerticalBorderedRows" class="border-b border-muted last:border-b-0"></tr>
          </template>
        </tbody>
      </table>
    </div>
    <div v-else class="w-full overflow-hidden rounded-md border border-muted">
      <table class="m-0 w-full table-auto border-collapse">
        <tbody>
          <tr
