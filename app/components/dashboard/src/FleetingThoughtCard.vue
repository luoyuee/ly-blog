...
const loadData = async () => {
  ...
  try {
    ...
    $notify.error({
      title: t("components.dashboard.fleetingThought.loadError"),
      error
    });
  } finally {
    state.loading = false;
...
onMounted(async () => {
  try {
    ...
    $notify.error({
      title: t("components.dashboard.fleetingThought.initError"),
      error
    });
  }
});
...
<template>
  ...
  >
    <div class="flex h-6 items-center justify-between leading-6">
      <span>{{ $t("components.dashboard.fleetingThought.title") }}</span>
    ...
    <div class="text-sm">
      {{ $t("components.dashboard.fleetingThought.today", { count: data.today }) }}
    </div>
  </div>
</template>
