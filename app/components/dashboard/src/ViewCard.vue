...
import { getViewDashboard } from "@/apis/dashboard";
...
const loadData = async () => {
  ...
  try {
    state.loading = true;
    data.value = await getViewDashboard();
  } catch (error) {
    $notify.error({
      title: t("components.dashboard.view.loadError"),
      error
    });
  } finally {
    state.loading = false;
...
onMounted(async () => {
  try {
    ...
    $notify.error({
      title: t("components.dashboard.view.initError"),
      error
    });
  }
});
...
<template>
  ...
  >
    <div class="flex h-6 items-center justify-between leading-6">
      <span>{{ $t("components.dashboard.view.title") }}</span>
    ...
    <div class="text-sm">{{ $t("components.dashboard.view.today", { count: data.today }) }}</div>
  </div>
</template>
