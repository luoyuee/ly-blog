...
const handleChangeTab = (e: EditorTabItem) => {
  ...
  if (e.type === "note") {
    mdcEditorEmitter.emit("cmd.editor-core:switch:file", e);
  }
};
...
const handleCloseTab = (e: EditorTabItem) => {
  ...
  mdcEditorEmitter.emit("cmd.editor-core:close:file", e);

  const next = mdcEditorStore.getCurrentTabItem();
  if (next && next.type === "note") {
    mdcEditorEmitter.emit("cmd.editor-core:switch:file", next);
  }
};
...
<template>
  <Scrollbar ref="scrollbarRef" :height="34" class="w-full" mouse-wheel="horizontal">
    <VueDraggable v-model="mdcEditorStore.tabs" target=".editor-tab-bar" :animation="150">
      <div class="editor-tab-bar" @wheel="handleWheel">
        <div
          v-for="item in mdcEditorStore.tabs"
          :key="item.key"
          class="editor-tab-bar__item"
          :class="{
            'editor-tab-bar__item--active': mdcEditorStore.currenTab === item.key,
