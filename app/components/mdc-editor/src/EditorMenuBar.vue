...
const togglePreview = () => {
  mdcEditorStore.preview.show = !mdcEditorStore.preview.show;
};

const toggleSidebar = () => {
  mdcEditorStore.sidebar.show = !mdcEditorStore.sidebar.show;
};
...
<template>
  <nav class="mdc-editor-menu-bar">
    ...
    <div class="menu-bar-window-controls">
      <UTooltip text="切换主侧栏" :content="{ side: 'bottom' }">
        <div @click="toggleSidebar">
          <UIcon
            :name="mdcEditorStore.sidebar.show ? 'custom:right-bar-fill' : 'custom:right-bar'"
          />
        </div>
      </UTooltip>
      <UTooltip text="切换辅助侧栏" :content="{ side: 'bottom' }">
        <div @click="togglePreview">
          <UIcon
            :name="mdcEditorStore.preview.show ? 'custom:right-bar-fill' : 'custom:right-bar'"
          />
        </div>
      </UTooltip>
    ...
    <div class="flex pl-2 pr-3 items-center">
      ...
      >
        <UAvatar src="https://github.com/benjamincanac.png" size="xs" class="cursor-pointer" />
      </UDropdownMenu>
    </div>
  </nav>
</template>
...
.mdc-editor-menu-bar {
  height: var(--mdc-editor-menu-bar-height);
