...
import type { HitokotoItem, HitokotoTypeItem } from "@/types/hitokoto";
...
const loadTypeOptions = async () => {
  try {
    ...
    typeMap.value = res.reduce(
      (map: Record<number, HitokotoTypeItem>, item: HitokotoTypeItem) => {
        map[item.id] = item;
        return map;
      },
      {}
  ...
  },
  {
    ...
    cell: ({ row }) =>
      h(
        "div",
        { class: "w-34" },
        dayjs(row.original.updated_at ?? row.original.created_at).format(
          "YYYY/MM/DD HH:mm:ss"
        )
      ),
  },
  ...
  <div class="p-4 h-full overflow-hidden flex flex-col">
    <div class="flex justify-between items-center mb-4">
      <div class="flex gap-4">
        <UButton icon="ep:plus" @click="handleOpenHitokotoFormModal()">
          新增
        </UButton>
      ...
      <UButtonGroup>
        <UInput v-model.trim="state.keyword" placeholder="请输入关键词" />
