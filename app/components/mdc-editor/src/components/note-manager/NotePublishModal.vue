    ...
    if (note.value !== null) {
      if (article.value) {
        await updatedPublishArticle({
          ...
          pinned: formData.enable_pinned ? formData.pinned : undefined,
          ...
          url_access_only: formData.url ? formData.url_access_only : false
        });
        $notify.success({
          title: "更新成功"
        });
...
mdcEditorEmitter.on("cmd.note-manager:publish:article", async (e: FolderTreeItem) => {
        ...
        <div class="flex gap-4">
          <div class="flex-1">
            ...
            <UFormField label="作者" name="author">
              <UInput v-model="formData.author" placeholder="洛月" />
            </UFormField>
            ...
            >
              <UInputNumber v-model="formData.pin_priority" />
            </UFormField>
          </div>
          <div class="flex-1">
            <UFormField label="封面图片" name="cover">
              <!-- <SelectImage v-model:value="formData.cover" multiple :multipleLimit="3" /> -->
            </UFormField>
            ...
            <Descriptions v-if="article" title="文章信息" border class="mb-4" :column="1">
              <DescriptionsItem label="创建时间">
                {{
                  article.created_at ? dayjs.unix(article.created_at).format($t("dateFormat")) : ""
                }}
              </DescriptionsItem>
              <DescriptionsItem label="更新时间">
                {{ dayjs.unix(article.content_updated_at).format($t("dateFormat")) }}
              </DescriptionsItem>
              <DescriptionsItem label="统计数据">
                <el-space>
                  <el-tag>{{ "阅读：" + article.view_count }}</el-tag>
                  <el-tag>{{ "评论：" + article.comment_count }}</el-tag>
                  <el-tag>{{ "点赞：" + article.like_count }}</el-tag>
                </el-space>
              </DescriptionsItem>
            ...
            <UAlert
              v-if="note && article && note.content_updated_at > article.content_updated_at"
              type="warning"
            >
              笔记内容已更新，需要同步
            </UAlert>
