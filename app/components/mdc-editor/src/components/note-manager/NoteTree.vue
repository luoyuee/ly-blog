...
const handleOpenFile = async (data: FolderTreeItem) => {
  ...
  if (data.type === "note") {
    try {
      ...
      mdcEditorEmitter.emit("cmd.editor-core:open:file", {
        key: path,
...
const handlePublishNote = (data: FolderTreeItem) => {
  console.log(data);
  mdcEditorEmitter.emit("cmd.note-manager:publish:article", data);
};
</script>
<template>
          ...
          >
            ...
            >
              <UIcon
                ...
                :name="item.is_expanded ? 'mdi:folder-open' : 'mdi:folder'"
              />
