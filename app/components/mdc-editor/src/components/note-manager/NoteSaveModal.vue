...
mdcEditorEmitter.on("intent.editor-core:save:file", async (e: EditorTabItem): Promise<void> => {
...
const handleSubmit = async (event: FormSubmitEvent<z.output<typeof schema>>) => {
  try {
    ...
    if (state.tabItem) {
      mdcEditorEmitter.emit("cmd.editor-core:update:file", {
        ...state.tabItem,
        isChange: false,
    ...
    state.visible = false;
    mdcEditorEmitter.emit("cmd.note-manager:reload");
  } catch (error) {
    $notify.error({
      title: "保存失败",
      error
