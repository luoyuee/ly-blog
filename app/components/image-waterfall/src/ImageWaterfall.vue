...
const columnWidth = computed(() => {
  return (
    (width.value - (columnCount.value - 1) * props.gap) / columnCount.value
  );
});
...
const checkImagesVisibility = () => {
  ...
  const imageElements = imageWaterfallRef.value.querySelectorAll(
...
<template>
  ...
  >
    ...
    >
      <img
        v-if="
          renderData.images[index] &&
          (!lazyLoad || renderData.images[index].visible)
        "
      ...
      <div
        v-if="
          !renderData.images[index]?.loaded && !renderData.images[index]?.error
        "
        class="image-loading"
      >
