...
lyEditorEmitter.on("editor-core:save", async (e: EditorTabItem): Promise<void> => {
...
const handleSubmit = async (event: FormSubmitEvent<z.output<typeof schema>>) => {
  try {
    ...
    // lyEditorEmitter.emit("monaco-editor:update-file", {
    //   ...state.data,
    //   isChange: false,
    // });
    ...
    state.visible = false;
    // lyEditorEmitter.emit("explorer:reload");

    // if (state.data && state.data.closeTab) {
    //   lyEditorEmitter.emit("monaco-editor:close-file", state.data);
    // }
  } catch (error) {
    $notify.error({
      title: "保存失败",
