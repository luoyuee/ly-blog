...
import { isArray, isDate, isMap, isObject, isRegExp, isSet } from "../typed";
...
export interface CloningStrategy {
  ...
  cloneRegExp?: (regex: RegExp) => RegExp;
  cloneError?: <E extends Error>(error: E) => E;
...
export const DefaultCloningStrategy: CloningStrategy = {
  ...
  cloneDate: (date: Date) => new Date(date.getTime()),
  cloneRegExp: (regex: RegExp) => {
    const cloned = new RegExp(regex.source, regex.flags);
    if ((regex as any).lastIndex) cloned.lastIndex = regex.lastIndex;
    return cloned;
  },
  cloneError: <E extends Error>(error: E) => {
    const Ctor = error.constructor as new (message?: string) => E;
    const cloned = new Ctor(error.message);
    cloned.stack = error.stack;
    cloned.name = error.name;
    return cloned;
  },
  cloneArrayBuffer: (buffer: ArrayBuffer) => buffer.slice(0),
...
export function cloneDeep<T extends object>(
  ...
  const cloneDeep = (parent: unknown, strategy: CloningStrategy): unknown => {
    // 特殊类型处理优先
    if (isDate(parent) && strategy.cloneDate) {
      return track(parent, strategy.cloneDate(parent));
    }
    if (isRegExp(parent) && strategy.cloneRegExp) {
      return track(parent, strategy.cloneRegExp(parent));
    }
    if (isError(parent) && strategy.cloneError) {
      return track(parent, strategy.cloneError(parent));
    }
    if (parent instanceof ArrayBuffer && strategy.cloneArrayBuffer) {
      return track(parent, strategy.cloneArrayBuffer(parent));
    }
